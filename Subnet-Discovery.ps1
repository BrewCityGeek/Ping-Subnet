...
$ScriptBlock = {
    ...
    try {
        ...
        [PSCustomObject]@{
            ...
            'owner'       = ""
            'device'      = ""
            'port'        = if ($using:Port -gt 0) { if ($portOpen) { "Open" } else { "Closed" } } else { "" }
            'note'        = ""
            'location'    = ""
            # Keep original properties for display compatibility
...
$sortedResults = $results | Sort-Object { [version]$_.'ip address' }
Clear-Host
...
if ($Port -gt 0) {
    $displayResults | Select-Object 'ip address', Status, hostname, mac, port | Format-Table -AutoSize
} else {
    $displayResults | Select-Object 'ip address', Status, hostname, mac | Format-Table -AutoSize
}

# Optionally export to CSV (respecting ShowOffline)
if ($OutputCsv) {
    # Select only the desired CSV columns, excluding unwanted properties
    $csvData = $displayResults | Select-Object 'ip address', 'ip state', 'description', 'hostname', 'fw_object', 'mac', 'owner', 'device', 'port', 'note', 'location'
